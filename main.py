...
# How many hourly points to publish (keeps keys names same; increases number of hour_N entries)
EXTENDED_HOURS = int(os.getenv("EXTENDED_HOURS", "12"))  # default 12; set to 24 to publish 24 hours
...
LLM_BACKOFF_SECONDS = int(os.getenv("LLM_BACKOFF_SECONDS", 3600))
LLM_MIN_INTERVAL = int(os.getenv("LLM_MIN_INTERVAL", 300))
...
# runtime state for LLM backoff/throttle
...
WEATHER_CODE_MAP = {
    ...
    86: "Tuyết rơi mạnh", 95: "Giông nhẹ hoặc vừa", 96: "Giông kèm mưa đá nhẹ",
...
def get_weather_forecast():
    ...
    try:
        ...
        if r.status_code != 200:
            ...
            # retry without explicit dates as fallback
            try:
        ...
        # Build next_hours but limit by EXTENDED_HOURS (user-configurable)
        next_hours = []
        for offset in range(0, max(EXTENDED_HOURS, 1)):
            i = idx + offset
            if i >= len(hour_times):
                break
            h = {
...
def update_bias_and_correct(next_hours, observed_temp):
    """
    Compute bias from history and insert new observation into history store.
    NOTE: we compute bias and persist history, but we DO NOT modify next_hours or create
    temperature_corrected fields (user requested not to include corrected fields).
    Returns (bias, next_hours) for downstream use.
    """
    global bias_history
    if not next_hours:
        # still record nothing but return zero bias
    ...
    # important: do NOT write any *_temperature_corrected keys here (user asked)
...
def call_openrouter_llm(system_prompt: str, user_prompt: str, model: str = LLM_MODEL, max_tokens: int = 400, temperature: float = 0.0):
    ...
    # propagate http errors to caller (they are handled there)
    r.raise_for_status()
    data = r.json()
    try:
...
def merge_weather_and_hours(existing_data=None):
    """
    Merge forecast into flattened telemetry.
    NOTE: Do NOT add any *_temperature_corrected keys here (per user request).
    """
    ...
    # hours: publish up to EXTENDED_HOURS but keep keys hour_0 ... hour_11 (or more)
    next_hours = weather.get("next_hours", []) or []
    for idx in range(0, EXTENDED_HOURS):
        h = next_hours[idx] if idx < len(next_hours) else None
        time_label = None
        if h and h.get("time"):
            try:
        ...
        # always write temperature/humidity/desc keys if we have values; if not, set None to keep keys visible
        if h:
            flattened[f"hour_{idx}_temperature"] = h.get("temperature")
            flattened[f"hour_{idx}_humidity"] = h.get("humidity")
            flattened[f"hour_{idx}_weather_desc"] = h.get("weather_desc")
        else:
...
def receive_data(data: SensorData):
    ...
    # update bias (we compute bias and persist history but do not create corrected fields)
    bias, _ = update_bias_and_correct(next_hours, data.temperature)
    ...
    # LLM integration: call if key set and not disabled/backoff and rate-limit observed
    llm_advice = None
    global llm_disabled_until, _last_llm_call_ts
    now_ts = time.time()
    if OPENROUTER_API_KEY and now_ts >= llm_disabled_until and (now_ts - _last_llm_call_ts) >= LLM_MIN_INTERVAL:
        try:
            # system and user prompts kept simple and safe (return JSON only)
            system_prompt = (
                "Bạn là một chuyên gia nông nghiệp và dự báo thời tiết. Trả về CHỈ MỘT đối tượng JSON ngắn gọn "
                "với các trường: advice (string ngắn, tiếng Việt), priority (low/medium/high), actions (mảng string), reason (string). "
                "KHÔNG kèm văn bản khác."
            )
            user_prompt = (
                f"Observed: temp={data.temperature}C, hum={data.humidity}%. Bias={bias}. "
                f"Next hours (up to {EXTENDED_HOURS}): {json.dumps(next_hours, ensure_ascii=False)}. Return JSON only."
            )
            ...
            # handle common HTTP errors and apply backoff for 402 (insufficient credits)
            ...
            try:
                ...
                pass
            logger.warning(f"LLM HTTP {status}: {body}")
            llm_advice = {"error": "http_error", "status": status, "body": body}
            if status == 402:
                # backoff LLM for a while
                llm_disabled_until = time.time() + LLM_BACKOFF_SECONDS
                logger.warning(f"LLM disabled for {LLM_BACKOFF_SECONDS} seconds due to 402 (insufficient credits).")
        except Exception as e:
            logger.warning(f"LLM call failed: {e}")
            llm_advice = {"error": "llm_failed", "reason": str(e)}
    else:
        # Skip LLM call and indicate reason
        reason = "OPENROUTER_API_KEY not set" if not OPENROUTER_API_KEY else "disabled/backoff_or_rate_limit"
        llm_advice = {"skipped": reason}

    merged["llm_advice"] = llm_advice

    # attach forecast next_hours (unchanged) and push telemetry
...
# ================== AUTO LOOP (simulator) ==================
async def auto_loop():
    logger.info("Auto-loop simulator started (LLM calls when enabled)")
    battery = 4.2
    while True:
        try:
            now = _now_local()
            ...
            bias, _ = update_bias_and_correct(next_hours, sample["temperature"])
            advice_data = get_advice(sample["temperature"], sample["humidity"], upcoming_weather=next_hours)
            ...
            # LLM call in auto-loop if enabled
            llm_advice = None
            global llm_disabled_until, _last_llm_call_ts
            now_ts = time.time()
            if OPENROUTER_API_KEY and now_ts >= llm_disabled_until and (now_ts - _last_llm_call_ts) >= LLM_MIN_INTERVAL:
                try:
                    system_prompt = (
                        "Bạn là một chuyên gia nông nghiệp. Trả về CHỈ MỘT JSON ngắn gọn với các trường: "
...
# - To change how many hours are published, set ENV EXTENDED_HOURS (e.g. 24)
# - Keys names (hour_0, hour_0_temperature, advice, ...) are preserved as requested.
