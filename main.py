...
def generate_sample_data():
    ...
    payload = {
        ...
        "time_sent": datetime.now().strftime("%Y-%m-%d %H:%M:%S"),  # thêm timestamp
...
async def receive_telemetry(req: Request):
    data = await req.json()
    # Thêm timestamp vào dữ liệu ESP32 để đồng nhất
...
if __name__ == "__main__":
    # Gửi dữ liệu mẫu ngay khi khởi động
    logger.info("[INIT] Gửi dữ liệu mẫu ngay khi khởi động server...")
